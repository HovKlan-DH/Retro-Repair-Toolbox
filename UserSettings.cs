    ...
    public static class UserSettings
    {
        ...
        public static void Load()
        {
            try
            {
                ...
                {
                    _data = loaded;
                    Logger.Info($"Settings loaded - [CheckVersionOnLaunch] [{CheckVersionOnLaunch}] [CheckDataOnLaunch] [{CheckDataOnLaunch}] [LeftPanelWidth] [{_data.LeftPanelWidth:F1}] [SchematicsSplitterRatios] [{_data.SchematicsSplitterRatios.Count} entries] [SelectedCategoriesByBoard] [{_data.SelectedCategoriesByBoard.Count} entries] [WindowPlacement] [{_data.WindowState}] [{_data.WindowWidth:F0}x{_data.WindowHeight:F0}]");
                }
            }
            catch (Exception ex)
            {
